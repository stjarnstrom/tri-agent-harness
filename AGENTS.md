...
The **runner** is Claude Code (`./harness.sh`, slash commands, subagents). Sibling
repos exist for other tools. Instructions and skills are still shared so they
do not fork per vendor:

| Path | Role |
|------|------|
| `AGENTS.md` | Canonical project instructions. Cursor, Codex, Copilot, and others read this natively. |
| `CLAUDE.md` | Claude Code loader: `@AGENTS.md`. Not a second copy — do not paste rules here. |
| `.agents/skills/` | Canonical skills (`SKILL.md`). The directory is **`.agents`** (plural), not `.agent`. |
| `.claude/skills` | Symlink → `.agents/skills` so Claude Code discovers the same skills. |
| `.claude/commands/`, `.claude/agents/`, `.claude/settings.json` | Claude Code adapters (slash commands, subagent defs, sandbox). Stay tool-specific. |

Edit `AGENTS.md` and `.agents/skills/`. Nested packages use the same pair:
`AGENTS.md` plus a one-line `CLAUDE.md` that imports it. `bun run setup`
recreates the skills symlink if a Windows checkout without `core.symlinks`
materialized it as a plain file.
...
- `/optimize-claude-md [path] [apply]` — Trim `AGENTS.md` (the canonical instructions; `CLAUDE.md` is only a loader). Cuts anything inferable from the codebase (or findable by a senior dev in ~20 min); keeps non-obvious decisions, conventions, and gotchas. Dispatches to the `claude-md-optimizer` subagent (Fable). Default writes `AGENTS.optimized.md` for review; `apply` rewrites in place.
