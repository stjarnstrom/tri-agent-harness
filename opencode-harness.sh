...
if harness_is_planning_complete; then
  ...
  run_opencode_agent planner 1 "$(harness_build_planner_prompt "$PROMPT")"
...
IMPORTANT: Pre-QA mechanical checks failed. Read docs/mechanical-checks-sprint-${CURRENT}.md and fix ALL listed issues."
    ...
    run_opencode_agent generator "$CURRENT" "$(cat agents/generator.md)
...
$HARNESS_AUTONOMOUS_SUFFIX"
    ...
    run_opencode_agent evaluator "$CURRENT" "$(cat agents/evaluator.md)
...
echo ""
echo "============================================"
echo "  OPENCODE HARNESS COMPLETE"
